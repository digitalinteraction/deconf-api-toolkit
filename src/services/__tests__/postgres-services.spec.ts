...
import { mockPostgresClient } from '../__mocks__'
...
function setup() {
  ...
  const service = createPostgresService('postgresql://user:host@localhost')
...
describe('PoolClient', () => {
  describe('release', () => {
    it('should release the client', async () => {
      const { service, pool, poolClient } = setup()
      mocked<any>(pool.connect).mockResolvedValue(poolClient)
      const client = await createPostgresClient(pool)
